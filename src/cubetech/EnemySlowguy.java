 ...
 * @author mads
 */
public class EnemySlowguy extends EnemyBase {
    ...
    public EnemySlowguy(int x, int y) {
...
//        System.out.println("SPEED: "+speed);
        ...
        if (len < 40){
            ...
            // Set velocity in units per second
            wishdir.x *= 15f;
            wishdir.y *= 15f;
            ...
            if(nextThinkTime <= 0) {
                if (speed < 10){
                    wishdir = new Vector2f((float)(Ref.rnd.nextFloat()-0.5f)*15,(float)(Ref.rnd.nextFloat()-0.5f)*15);
                    Vector2f.add(vel, wishdir, vectorAdd);
                    setVelocity(vectorAdd);
                }else{
                    setVelocity(new Vector2f(vel.x*0.8f,vel.y*0.8f));
                }
                
                if (vectorAdd.y < 0){
                    spriteAngle=-(float)Math.atan((vectorAdd.x/vectorAdd.y));
                }else{
                    spriteAngle=-(3.14f+(float)Math.atan((vectorAdd.x/vectorAdd.y)));
                }
//                System.out.println(vectorAdd+"    "+spriteAngle);
                nextThinkTime = 1500; // dumbest fuck
            }
        }
    ...
    @Override
    public void Render() {
        CubeTexture tex = (CubeTexture)(Ref.ResMan.LoadResource("data/enemy5.png").Data);
